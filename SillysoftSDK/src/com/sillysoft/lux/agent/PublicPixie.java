...
public class PublicPixie extends SmartAgentBase  {
	protected float outnumberBy = 1;	// for individual country attacks
	protected int borderForce = 20;
	protected boolean[] ourConts;	// whether we will spend efforts taking/holding each continent
...
// NOTE: This will not check countries that are in the same continent as 'center'
	...
	public int moveArmiesIn( int cca, int ccd) {
		...
		if (ourConts[ countries[cca].getContinent() ] && ourConts[ countries[ccd].getContinent() ]) {	// we want both
			return countries[cca].getArmies()/2;
	...
	public void fortifyPhase() {
		GameState state = new GameState(countries, GameState.GamePhase.Fortify, ID);
